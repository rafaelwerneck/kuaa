...
def train_test(images, classes, parameters):
    ...
    to the parameters:
        ...
        - Train Size: Value of the division of the set. Select the number of images
            of the database that will be part of the train set;
        - Test Size: Value of the division of the set. Select the number of images
            of the database that will be part of the test set;
    """
    
    print "Train and Test: Number of Images"
    ...
    if param_classes:
        for id_class in classes.iterkeys():
            ...
            try:
                ...
                if param_train_size >= len(classes[id_class]):
                    class_train = classes[id_class]
                    class_test = []
                elif param_test_size >= len(classes[id_class]):
                    class_train = []
                    class_test = classes[id_class]
                else:
                    #In case that the sum of the values in the train set and
                    # the test set be over the number of images available, half
    ...
    #Considering the dataset as whole
        ...
        try:
            ...
            if param_train_size >= len(images.keys()):
                class_train = images.keys()
                class_test = []
            elif param_test_size >= len(images.keys()):
                class_train = []
...
def write_tex(method, parameters):
    ...
    tex_string = """
\\section{Protocol: Number of Images}
\\begin{table}[htbp]
    \\centering
    \\begin{tabular}{cc}
        Parameter & Value \\\\
        ...
        Test Size & %d \\\\
    \\end{tabular}
    \\caption{Parameters of the Number of Images Method.}
    \\label{tab:%s_param_%s_%d_%d}
\\end{table}
