...
def evaluation(images, test_set, classes_list, pos_train_test, parameters):
    ...
    not from this class, and was not classified as this class.
    """
    ...
    #For each class, calculate the true negative
    for item_class in interest_classes:
        ...
        results[item_class] = [true_negative, true_negative / all_images]
    
    #-------------------------------------------------------------------------
    print "End of the True Negative method."
...
def string_file(true_negative):
    """
    Returns a one line dictionary with the true negative to be write into the
    file.
    """
    
    return str(true_negative)
...
def tex_name():
    ...
    print "\True Negative"
    return "\\subsection{True Negative}"

def write_tex(evaluation_path, classes, node_id):
    """
    Calculates an average number of true negatives in the experiment.
    """
    ...
    print "\t\tTeX: True Negative"
    
    tn_dicts = []
    
    evaluation_file = open(evaluation_path, "rb")
    for line in evaluation_file.readlines():
        tn_dicts.append(literal_eval(line))
    evaluation_file.close()
    
    avg_tn = {}
    for class_key in tn_dicts[INDEX_ZERO].iterkeys():
        tn_list = []
        perc_list = []
        for tn in tn_dicts:
            tn_list.append(tn[class_key][INDEX_TN])
            perc_list.append(tn[class_key][INDEX_PERC])
        avg_tn[class_key] = [array(tn_list).mean(), array(perc_list).mean()]
    
    evaluation_file = open(evaluation_path, "ab")
    evaluation_file.write("\nAverage True Negative\n")
    evaluation_file.write(str(avg_tn))
    evaluation_file.close()
...
\\begin{table}[htbp]
    \\centering
    \\begin{tabular}{ccc}
        & Number of True Negatives & Percentage of Total (\\%) \\\\
        \\hline"""
    
    for class_key in avg_tn.iterkeys():
        tex_string += """
        %s & %.2f & %.2f \\\\""" % (class_key, avg_tn[class_key][INDEX_TN],
                avg_tn[class_key][INDEX_PERC] * 100)
                
    tex_string += """
    \\end{tabular}
    \\caption{Mean Number of True Negatives and Percentage of True Negatives of Node %s}
    \\label{tab:tn_%s}
\\end{table}
    """ % (node_id, node_id)
