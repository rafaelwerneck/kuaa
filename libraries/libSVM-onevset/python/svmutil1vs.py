...
def svm_predict(y, x, m, options=""):
	...
	def info(s):            # pedrormjunior 20131021
		print(s)
	...
	while i < len(argv):
		...
		elif argv[i] == '-q': # pedrormjunior 20131021
			info = print_null
		else:
	...
	if predict_probability:
		...
		if svm_type in (ONE_CLASS, EPSILON_SVR, NU_SVC):
			...
			nr_classifier = nr_class*(nr_class-1)//2
                dec_values = (c_double * (nr_classifier+1))()
		for xi in x:
			xi, idx = gen_svm_nodearray(xi)
			label = libsvm.svm_predict_values(m, xi, dec_values)
# 			values = dec_values[:nr_classifier]
# 			values = [dec_values[1]] # pedrormjunior 20131028
			values = [dec_values[1], dec_values[0]] # pedrormjunior 20131028
#                         print dec_values[:]
			pred_labels += [label]
			pred_values += [values]
