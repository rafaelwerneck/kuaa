...
elif find_library('lib1vs'):
	libsvm = CDLL(find_library('lib1vs'))
else:
...
OPENSET_OPTIMIZATION_T = ['OPT_PRECISION', 'OPT_RECALL',  'OPT_FMEASURE',  'OPT_HINGE', 'OPT_BALANCEDRISK'] # pedrormjunior 20131023
for i, s in enumerate(SVM_TYPE): exec("%s = %d" % (s , i))
for i, s in enumerate(KERNEL_TYPE): exec("%s = %d" % (s , i))
for i, s in enumerate(OPENSET_OPTIMIZATION_T): exec("%s = %d" % (s , i)) # pedrormjunior 20131023
...
# 	_types = [c_int, POINTER(c_double), POINTER(POINTER(svm_node))]
	...
	_types = [c_int, POINTER(c_double), POINTER(POINTER(svm_node)), c_int, POINTER(c_int)]
	_fields_ = genFields(_names, _types)
...
class svm_parameter(Structure):
        _names = ["svm_type", "kernel_type", "do_open", "degree", "gamma", "coef0", "cache_size", "eps", "C", "nr_weight", "nr_fold", "cross_validation", "weight_label", "weight", "nu", "p", "shrinking", "probability", "neg_labels", "exaustive_open", "optimize", "beta", "near_preasure", "far_preasure", "openset_min_probability", "rejectedID", ]

        _types = [c_int, c_int, c_int, c_int, c_double, c_double, c_double, c_double, c_double, c_int, c_int, c_int, POINTER(c_int), POINTER(c_double), c_double, c_double, c_int, c_int, c_bool, c_bool, c_int, # openset_optimization_t,
...
# 			c_double, c_double, c_int, c_int]
	...
	def set_to_default_values(self):
		...
		self.print_func = None
                # pedrormjunior 20131023: Setting up the default
                # parameters of the SVM1VS:
                self.do_open = 1
                self.neg_labels = True # Default is False.
                self.exaustive_open = False
                self.optimize = OPT_FMEASURE
                self.beta = 1.0
                self.near_preasure = 1.6
                self.far_preasure = 4.0
                self.openset_min_probability = 0.0 # for PIESVM
	...
	def parse_options(self, options):
		...
		while i < len(argv):
			if argv[i] == "-s":
				...
				weight += [float(argv[i])]
			elif argv[i] == "-G": # pedrormjunior 20131025
				i = i + 1
...
# 			POINTER(c_int), c_int]
	_names = ['param', 'nr_class', 'l', 'SV', 'sv_coef', 'rho', 'probA', 'probB', 'openset_dim', 'alpha', 'omega', 'wbltrans', 'wblshape', 'wblscale', 'label', 'nSV', 'free_sv']
	_types = [svm_parameter, c_int, c_int, POINTER(POINTER(svm_node)), POINTER(POINTER(c_double)), POINTER(c_double), POINTER(c_double), POINTER(c_double), c_int, POINTER(c_double), POINTER(c_double), POINTER(c_double), POINTER(c_double), POINTER(c_double), POINTER(c_int), POINTER(c_int), c_int]
	_fields_ = genFields(_names, _types)
